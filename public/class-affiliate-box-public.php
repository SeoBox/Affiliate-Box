	...
	public function acf_init() {
		...
		if (function_exists('acf_register_block')) {
			acf_register_block(array(
				'name'            => 'roundup-summary', 'title' => 'Roundup Review Summary',
				...
				'category'        => 'acfpr-blocks', 'icon' => 'text', 'post_types' => array('post', 'page'),
			...
			acf_register_block(array(
				'name'            => 'roundup-reviews', 'title' => 'Elementor Roundup Reviews',
				...
				'category'        => 'acfpr-blocks', 'icon' => 'star-half', 'post_types' => array('post', 'page'),
				'keywords'        => array('roundup', 'review', "product"), 'enqueue_style' => $enqueue_style,
			));


			acf_register_block(array(
				'name'            => 'roundup-review-templates', 'title' => 'Roundup Reviews',
				'description'     => 'Renders products reviews. Visible only when a post is published.',
		...
		if (function_exists('acf_add_options_page') && current_user_can('manage_options')) {
			...
			$parent = acf_add_options_page(array(
				'menu_title' => 'Product Review Settings', 'menu_slug' => 'product-review-settings',
				'capability' => 'manage_options', 'redirect' => true,
			));

			acf_add_options_sub_page(array(
				'page_title' => 'General Settings', 'menu_title' => 'General Settings',
				'capability' => 'manage_options', 'parent_slug' => $parent['menu_slug'],
			));
			acf_add_options_sub_page(array(
				'page_title' => 'Affiliate Settings', 'menu_title' => 'Affiliate Settings',
				'capability' => 'manage_options', 'parent_slug' => $parent['menu_slug'],
			));
		}
	}
...
// Add Custom Blocks Panel in Gutenberg
	function block_categories($categories, $post) {
		return array_merge($categories, array(
			array(
				'slug' => 'acfpr-blocks', 'title' => __('Product Reviews', 'acfpr-blocks-master'),
			),
		));
	}
	...
	function afc_load_value_reviews($value, $post_id, $field) {
		...
		foreach ($parsing_logics as $parsing_logic) {
			...
			foreach ($products as $product) {
				$new_row = array(
					'field_5e0821999c85e' => $product->asin, 'field_5e092db33655d' => $product->title,
					'field_5e084bbd32476' => $product->description, 'field_5e5744c7300f8' => $product->bestCategory,
					'field_5e2bc46c572b4' => implode("\n", $product->specs),
					'field_5e0908c36812b' => implode("\n", $product->pros),
					'field_5e0908d56812c' => implode("\n", $product->cons)
				);

				if ( ! empty($product->productLinks)) {
					$new_links = array();
					foreach ($product->productLinks as $anchor => $link) {
						array_push($new_links, array(
							'field_5ed0a4bea5587' => $link, 'field_5ed3158d3b984' => $anchor
						));
					}
					$new_row['field_5ed0a4a7a5586'] = $new_links;
				}
				$value[] = $new_row;
			}
